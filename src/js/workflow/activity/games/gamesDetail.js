define(['Util','text!module/workflow/activity/actDetailsPage.tpl'
              ...
              ,'text!module/workflow/activity/games/gamesDetail.tpl'
    ],
    function (Util,actPageTpl,actPageInfoTpl,actTableTpl) {
        ...
        function loadData() {   //加载数据
            Util.ajax.postJson("front/sh/game!execute?uid=m001&cmpgnId=" + _param.actId,"",function (json,status) {
                if(status){
                    var bean = json.bean;
                    var object = json.object;
                    Util.ajax.loadTemp($("#G-details-actInfo",$el),actPageInfoTpl,{'bean':bean});  //渲染抽奖活动信息
                    Util.ajax.loadTemp($("#G-details-resInfo",$el),actTableTpl,{'beans':formatData(object),"imgUrl":bean.ftpUrl,'givprzType':bean.givprzTypeCd});  //渲染抽奖详细信息
                }
            })
        }
        //格式化服务器返回的数据
