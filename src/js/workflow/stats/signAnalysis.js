define([
        ...
        'text!module/workflow/stats/actAnalysis/signAnalysis/actStatsInfo.tpl',
        'text!module/workflow/stats/actAnalysis/signAnalysis/actResStatsInfo.tpl'],
    function (Util,lDate,times,detailsTpl,basicTpl,statsTpl,resTpl) {
        ...
        var basicInfo = function(bTime,eTime){
            ...
            Util.ajax.postJson("front/sh/campaign!execute?uid=tj008", param, function(json,status){
                if(status){
                    ...
                    Util.ajax.loadTemp($('#G-details-statsInfo',$el), statsTpl,{"beans":beans});
                    //资源
        ...
        var resourcesInfo = function(bTime,eTime){
            ...
            Util.ajax.postJson("front/sh/sign!execute?uid=si008",paramData, function(json,status){
                if(status){
                    var beans = json.beans;
                    for(var i = 0;i<beans.length;i++){
                        var _statNum = beans[i].statNum.split(',');
                        beans[i].statNum = _statNum;
                    }
                    Util.ajax.loadTemp($('#G-act-resStats',$el), resTpl, {"beans":beans,"times": times.getDayArr(bTime,eTime)});
                }
            });
        };
