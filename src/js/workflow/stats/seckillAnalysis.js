define([
        ...
        'text!module/workflow/stats/actAnalysis/seckillAnalysis/actStatsInfo.tpl',
        'text!module/workflow/stats/actAnalysis/seckillAnalysis/actResStatsInfo.tpl'],
    function (Util,lDate,times,detailsTpl,basicTpl,statsTpl,resTpl) {
        ...
        var basicInfo = function(getSecTime){
            var param = {
                cmpgnId: _param.param.actId,
                cmpgnTypeCd: _param.param.activityType,
                beginDate: getSecTime[0],
                endDate: getSecTime[1]
            };
            Util.ajax.postJson("front/sh/campaign!execute?uid=tj008", param, function(json,status){
                if(status){
                    ...
                    Util.ajax.loadTemp($('#G-details-statsInfo',$el), statsTpl,{"beans":beans,"imgUrl":bean.ftpUrl});
                    //资源
                    resourcesInfo(getSecTime[0],getSecTime[1]);
                }
            });
        };
        //时间插件
        ...
        var resourcesInfo = function(bTime,eTime){
            ...
            /*var paramData2 = {
                'cmpgnId':'20161207182811335584',
                'bgnValidTime':'2016-12-01',
                'endValidTime' :'2016-12-09'
            };*/
            Util.ajax.postJson("front/sh/campaign!execute?uid=os004",paramData, function(json,status){
                if(status){
                    var beans = json.beans;
                    //取出不重复的资源名称rsNm
                    var rsNm =[],
                        compare = {};
                    for(var z = 0;z<beans.length;z++){
                        if(!compare[beans[z].rsNm]){
                            rsNm.push(beans[z].rsNm);
                            compare[beans[z].rsNm] = 1;
                        }
                    }
                    var timesArray = times.getDayArr(paramData.bgnValidTime,paramData.endValidTime);
                    Util.ajax.loadTemp($('#G-act-resStats',$el), resTpl,{"times":timesArray,"beans":beans,"rsNms":rsNm});
                    //去除多余数据
                    var timesArrayLen = timesArray.length;
                    for(var i = 7;i>timesArrayLen;i--){
                        $('.day'+i,$el).remove();
                    }
